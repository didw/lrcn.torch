...
function M.parse(arg)
    ...
    cmd:option('-dataType', 'avi', 'jpg | avi')
    cmd:option('-data', '../../data/UCF-101', 'Home of ImageNet dataset')
    ...
    cmd:option('-epochSize',  2400000/16/16/2,    'Number of batches per epoch(total number / stride / batchSize)') 
    ...
    cmd:option('-trainType',   'transfer', 'none | transfer | finetune')
    cmd:option('-testType',        1, '1: test on batch frames randomly selected | 2: test on one video')
    cmd:text()
